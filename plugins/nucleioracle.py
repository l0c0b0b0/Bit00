...
class NucleiOracle:
    ...
    async def run(target, tag, output, service, protocol, port, module):
        ...
        cmd = f"/usr/bin/nuclei -no-color -silent -no-interactsh -target {target}:{port} -tags oracle,cve,misconfig,exposure -rate-limit 50 -concurrency 5 -retries 2 -max-host-error 2 -no-httpx -o {output}/scans/{protocol}_{port}_{service}_nuclei.txt"
