...
class NucleiHttp:
    ...
    async def run(target, tag, output, service, protocol, port, module):
        ...
        """Run nuclei scan."""
        cmd = f'/usr/bin/nuclei -no-color -silent -no-interactsh -target {service}://{target}:{port} -tags http,https -rate-limit 50 -concurrency 10 -retries 2 -max-host-error 2 -o {output}/scans/{protocol}_{port}_{service}_nuclei.txt'
