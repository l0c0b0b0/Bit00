...
class NucleiDistcc:
    ...
    async def run(target, tag, output, service, protocol, port, module):
        ...
        cmd = f"/usr/bin/nuclei -no-color -silent -no-interactsh -target {target} -tags distccd,cve,misconfig,exposure -rate-limit 50 -concurrency 5 -retries 2 -max-host-error 2 -o {output}/scans/{protocol}_{port}_{service}_nuclei.txt"
